...
import { basicSchema } from "../../schemas/register";
import { useContext,useEffect } from "react";
import { useAuth } from "../../context/authProvider";
import { useState } from "react";
import {useNavigate} from 'react-router-dom';

const onSubmit = async (values, actions, signUp) => {
  actions.resetForm();
};

function Form() {
  const { signUp, auth, loading,error } = useAuth();
  ...
  return (
    <div className="signin-form">
      ...
      <form className="" onSubmit={handleSubmit}>
        ...
        <div className="signin-pass">
          <label htmlFor="">Şifrəni tesdiqle</label>
          <input
            type="password"
      ...
      {error &&
      <p style={{ color: "red",margin:"10px 0px 0px 0px"}}>There was an accaunt with this e-mail or number.</p>
      }
      </form>
    </div>
  );
